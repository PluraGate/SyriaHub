...
"[project]/RiderProjects/SyriaHub/app/api/saved-references/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { data: references, error } = await supabase.from('saved_references').select('*').eq('user_id', user.id).order('created_at', {
            ascending: false
        });
        if (error) {
...
async function POST(request) {
    try {
        ...
        const { data: existing } = await supabase.from('saved_references').select('id').eq('user_id', user.id).eq('url', url).single();
        if (existing) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$RiderProjects$2f$SyriaHub$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'Already saved',
                existing: true
        ...
        const { data: reference, error } = await supabase.from('saved_references').insert({
            user_id: user.id,
            title,
            url,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__73bc42d5._.js.map