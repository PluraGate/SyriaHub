...
"[project]/RiderProjects/SyriaHub/app/api/saved-searches/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: searches, error } = await supabase.from('saved_searches').select('*').eq('user_id', user.id).order('created_at', {
            ascending: false
        });
        if (error) {
...
async function POST(request) {
    try {
        ...
        const { data: search, error } = await supabase.from('saved_searches').insert({
            user_id: user.id,
            query,
            filters: filters || {},
...
async function PATCH(request) {
    try {
        ...
        const { data: search, error } = await supabase.from('saved_searches').update(updates).eq('id', id).eq('user_id', user.id).select().single();
        if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8725a2da._.js.map